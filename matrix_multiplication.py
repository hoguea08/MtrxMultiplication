...
def eliminate(matrix, src_row_index, num_cols):
    ...
    '''
    for column in range(0, num_cols):
        if ((matrix[column][src_row_index] != 0) and \
                (matrix[column][src_row_index] != 1)):
            value = matrix[column][src_row_index]
    if (src_row_index != len(matrix[0])-1):
        for j in range(0, num_cols):
            matrix[j][src_row_index+1] = matrix[j][src_row_index+1] \
                                         - value*matrix[j][src_row_index]
    else:
        for j in range(0, num_cols):
            matrix[j][src_row_index-1] = matrix[j][src_row_index-1] \
                                         - value*matrix[j][src_row_index]
...
def get_phi_hat(matrix):
    ...
    print("phi_hat = {:<} + {:<} * x + {:<} * x^2".format(c_0, c_1, c_2))
